...
note: BFS + PQ，难点在于在处理main node之后如何处理sub-node的reachable
...
and cnti is the total number of new nodes that you will subdivide the edge
into. Note that cnti == 0 means you will not subdivide the edge.

To subdivide the edge [ui, vi], replace it with (cnti + 1) new edges and cnti
new nodes. The new nodes are x1, x2, ..., xcnti, and the new edges are
...
class Solution:
    def reachableNodes(self, edges: List[List[int]], maxMoves: int, n: int) -> int:
        ...
        queue = [(0, 0)]  # cost, nodeid

        visited = set()
        dist = {}  # min dist from node 0 to node i
        ...
        res = len(visited)  # main-node
        # calculate the sub-nodes among each edge
        for u, v, w in edges:
            sum = 0
            if u in visited:
                sum += maxMoves - dist[u]
