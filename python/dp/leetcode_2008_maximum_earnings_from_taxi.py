...
note: sort by end times, binary search + dp, the same as 1235 Maximum Profit in Job Scheduling
...
class Solution:
    def maxTaxiEarnings(self, n: int, rides: List[List[int]]) -> int:
        ...
        end_time_rides = [ride[1] for ride in rides]
        dp = [0] * m
        dp[0] = rides[0][1] - rides[0][0] + rides[0][2]
        for i in range(1, m):
            cur = bisect.bisect_right(end_time_rides, rides[i][0])
            dp[i] = max(dp[i-1], rides[i][1] - rides[i][0] + rides[i][2] + (0 if cur == 0 else dp[cur-1]))
        return dp[-1]
