...
the final array has its elements in lexicographic order (i.e.,
strs[0] <= strs[1] <= strs[2] <= ... <= strs[n - 1]). Return the
...
class Solution:
    def minDeletionSize(self, A: List[str]) -> int:
        ...
        for j in range(n):
            not_sorted = False
            for i in range(m-1):
                if not sorted[i] and A[i][j] > A[i+1][j]:
                    res += 1
                    not_sorted = True
                    break
            if not_sorted:
            ...
            num_sorted = 0
            for i in range(m-1):
                sorted[i] = sorted[i] or A[i][j] < A[i+1][j]
                if sorted[i]:
                    num_sorted += 1
            if num_sorted == len(sorted):
                break
        return res
