...
note: 这道题本质是两个区间求交集，按照最少和最多的方式吃来确定是否当天可以吃到t种水果
比赛当天，没想清楚上下界的开闭。
...
answer[i] is true if you can eat a candy of type favoriteTypei on day
...
2- You can eat at most 4 candies each day.
   ...
   On day 2, you can only eat 4 candies (type 1 and type 2), so you cannot eat
   a candy of type 4 on day 2.
...
class Solution:
    def canEat(self, candiesCount: List[int], queries: List[List[int]]) -> List[bool]:
        ...
        pre_sum = [0]
        for i in range(n):
            pre_sum.append(pre_sum[-1] + candiesCount[i])

        res = []
        for t, d, cap in queries:
            candy_lower = pre_sum[t]+1  # forget + 1
            candy_upper = pre_sum[t+1]
            eat_lower = d + 1
            eat_upper = d * cap + cap
            if eat_upper < candy_lower or candy_upper < eat_lower:
                res.append(False)
            else:
                res.append(True)
        return res
