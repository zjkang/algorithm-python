...
[0, w.length - 1]. pickIndex() should return the integer proportional to its weight in the w array.
...
class Solution:
    ...
    def __init__(self, w: List[int]):
        ...
        for idx, val in enumerate(w):
            self.pre_sum[idx+1] = self.pre_sum[idx] + val 

    def pickIndex(self) -> int:
        import random
        # binary search based on pre-sum
        total = self.pre_sum[-1]
        num = random.randint(0, total-1)
        index = bisect.bisect_right(self.pre_sum, num)
        return index-1
