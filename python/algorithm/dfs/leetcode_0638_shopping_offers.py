...
note: dfs + state compression可以使用或者使用tuple,有点类似coin change
...
class Solution:
    def shoppingOffers(self, price: List[int], special: List[List[int]], needs: List[int]) -> int:
        # 1. can optimize with valid special
        # 2. encode needs into state integer
        special2 = []
        n = len(price)
        for s in special:
            total = 0
            ...
            if total > s[-1]:
                special2.append(s)
        self.memo = {}
        res = self.dfs(price, special2, needs)
        return res

    def dfs(self, price, special, needs):
        t_needs = tuple(needs)
        if t_needs in self.memo:
            return self.memo[t_needs]
        n = len(price)
        res = 0
        for i in range(n):
            res += price[i] * needs[i]
        ...
        for comb in special:
            if not self.can_use(needs, comb):
                continue
            al_needs = needs[:]
            for i in range(n):
                al_needs[i] -= comb[i]
            res = min(res, comb[-1] + self.dfs(price, special, al_needs))
        self.memo[t_needs] = res
        return res

    def can_use(self, needs, comb):
        n = len(needs)
        for i in range(n):
            if needs[i] < comb[i]:
                return False
