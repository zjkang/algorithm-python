...
[1,2,3,4,10,-2] is incorrect because they are not in the same order as
in groups. [10,-2] must come before [1,2,3,4].
...
class Solution:
    ...
    def canChoose(self, groups: List[List[int]], nums: List[int]) -> bool:
        ...
        while start < len(nums):
            if start + len(groups[i]) > len(nums):
                return False
            if self.check(nums, start, groups[i]):
                start += len(groups[i])
                i += 1
            else:
                start += 1
    ...
    def check(self, nums, start, group):
        i = 0
        while i < len(group):
